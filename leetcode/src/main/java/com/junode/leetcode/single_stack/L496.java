 ...
 * @version 1.0.0
 * @Description 下一个最大元素
 * @createTime 2024年06月10日 23:05:00
 */
public class L496 {
    ...
    public void test1() {
        ...
        for (int i = 0; i < nums2.length; i++) {
            while (!deque.isEmpty() && nums2[deque.peek()] < nums2[i]) {
                if (temp.get(nums2[deque.peek()]) != null) {
                    result[temp.get(nums2[deque.peek()])] = nums2[i];
                }
                deque.pop();
            }
            deque.push(i);
    ...
    public void test() {
        ...
        for (int i = 0; i < nums2.length; i++) {
            while (!deque.isEmpty() && deque.peek() < nums2[i]) {
                temp.put(deque.pop(), nums2[i]);
            }
            deque.add(nums2[i]);
        }
        for (int i = 0; i < nums1.length; i++) {
            result[i] = temp.getOrDefault(nums1[i], -1);
        }
